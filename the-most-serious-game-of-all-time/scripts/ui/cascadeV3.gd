...
func _subtween_hitzone_in(node: Node, delay: float) -> Tween:
	...
	t.tween_property(node, "scale", target_scale, tween_in_time).set_delay(delay)
...
func _subtween_hitzone_out(node: Node) -> Tween:
	...
	t.tween_property(node, "position", target_pos, maxf(tween_in_time * 0.7, 0.3))
	t.tween_property(node, "scale", Vector2(-2, 0), maxf(tween_in_time * 0.7, 0.3))
